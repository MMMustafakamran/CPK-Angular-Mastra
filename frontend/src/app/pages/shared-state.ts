  ...
  template: `
    ...
    <div class="space-y-6">
      ...
      <ui-callout title="The agent has to agree on the state shape">
        ...
        <code>{{ '{' }} notes, priority {{ '}' }}</code>. The Mastra agent in
        <code>backend/src/mastra/</code> has to carry that same shape in its
        working memory, and expose it to the model so it can both read and update
